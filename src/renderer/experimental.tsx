...
import { Config } from "@ledgerhq/live-common/lib/api/explorerConfig/types";
...
const deltaExperimentalExplorers = Object.keys(explorerConfig)
  ...
  .filter(Boolean) as [CryptoCurrency, Config][];
          ...
          description: (
            <Trans i18nKey="settings.experimental.features.experimentalExplorers.description">
              {deltaExperimentalExplorers
                .map(
                  (explorer) => {
                    const [currency, config] = explorer ?? [];

                    return (currency.isTestnetFor ? "t" : "") +
                    currency.ticker +
                    " " +
