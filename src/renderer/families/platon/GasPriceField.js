...
const FeesField = ({ account, transaction, status, updateTransaction }: Props) => {
  invariant(transaction.family === "platon", "FeeField: platon family expected");
  ...
  if (status.errors.gasPrice?.name === 'NotEnoughGas') {
    status.errors.gasPrice.name = 'NotEnoughBalance';
  }
  ...
  return (
    <FeeSliderField
      ...
      unit={units.length > 3 ? units[3] : units[0]}
