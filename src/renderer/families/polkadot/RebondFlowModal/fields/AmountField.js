...
const AmountField = ({
  ...
  invariant(
    account && transaction && account.polkadotResources,
    "polkadot account and transaction required",
  );

  const bridge = getAccountBridge(account, parentAccount);

  const defaultUnit = getAccountUnit(account);
  const { unlockingBalance = BigNumber(0) } = account.polkadotResources || {};
  ...
  const amountUnlocking = useMemo(
    () =>
      formatCurrencyUnit(defaultUnit, unlockingBalance, {
        disableRounding: true,
        showAllDigits: false,
        showCode: true,
        locale,
      }),
    [unlockingBalance, defaultUnit, locale],
  );
    ...
    <Box vertical flow={1}>
      <Label>
        ...
        <Text style={{ flex: 1 }} textAlign="right">
          <Trans i18nKey="polkadot.rebond.steps.amount.unlocking" values={{ amountUnlocking }} />
        </Text>
      </Label>
      <InputCurrency
        autoFocus={false}
