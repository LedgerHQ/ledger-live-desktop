...
const AmountField = ({
  ...
  const unlockableBalance = lockedBalance.minus(unlockingBalance);
  ...
  const amountUnlockable = useMemo(
    () =>
      formatCurrencyUnit(defaultUnit, unlockableBalance, {
        disableRounding: true,
        showAllDigits: false,
        showCode: true,
        locale,
      }),
    [unlockableBalance, defaultUnit, locale],
  );
    ...
    <Box vertical flow={1}>
      <Label>
        ...
        <Text style={{ flex: 1 }} textAlign="right">
          <Trans i18nKey="polkadot.unbond.steps.amount.unlockable" values={{ amountUnlockable }} />
        </Text>
      </Label>
      <InputCurrency
        autoFocus={false}
