...
const cells = Array.from({ length: 47 }, (_, i) => {
  ...
  return (
    ...
    >
      {i + 2}
      <div style={{ fontSize: "10px" }}>{colSpan > 0 ? `(column span ${colSpan})` : ""}</div>
      <div style={{ fontSize: "10px" }}>{rowSpan > 0 ? `(row span ${rowSpan})` : ""}</div>
    </Cell>
  );
});

// eslint-disable-next-line
const DefaultTemplate = ({ gridColumn, gridRow, gridArea, ...args }) => {
  return (
    <>
...
// eslint-disable-next-line
const FixedTemplate = ({ gridColumn, gridRow, gridArea, ...args }) => (
  <>
    <div style={{ marginBottom: "1em" }}>
