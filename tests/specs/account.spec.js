...
describe("Account", () => {
  ...
  describe("account operations list", () => {
    it("show the first account", async () => {
      ...
      await firstAccountRow.click();
      expect(await app.client.screenshot()).toMatchImageSnapshot({
        customSnapshotIdentifier: "account-operation-list",
      });
    });
    it("show the first operation", async () => {
      const listOfOperations = await $("#operation-list");
      const operationRows = await listOfOperations.$$(".operation-row");
      ...
      await modalPage.isDisplayed();
      expect(await app.client.screenshot()).toMatchImageSnapshot({
        customSnapshotIdentifier: "account-operation-details",
      });
      await modalPage.close();
    });
  });
